...
export const Login: React.FC<{ onToggle?: () => void }> = ({ onToggle }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Invalid credentials for Student portal');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ minHeight: '100vh', background: '#0a0a0c', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div className="glass fade-in" style={{ width: '100%', maxWidth: '400px', padding: '40px', borderRadius: '30px', border: '1px solid rgba(255,255,255,0.05)' }}>
        <div style={{ textAlign: 'center', marginBottom: '35px' }}>
          <div style={{ background: 'linear-gradient(135deg, #3b82f6 0%, #6366f1 100%)', width: '60px', height: '60px', borderRadius: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px' }}>
            <BookOpen size={30} color="white" />
          </div>
          <h1 style={{ fontSize: '24px', fontWeight: '800', marginBottom: '8px', letterSpacing: '-0.5px' }}>STUDENT PORTAL</h1>
          <p style={{ opacity: 0.5, fontSize: '14px' }}>Join your live academic sessions</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div>
            <label style={{ display: 'block', marginBottom: '8px', fontSize: '13px', opacity: 0.6, fontWeight: '600' }}>Student Email</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="input" placeholder="student@academy.com" required style={{ width: '100%' }} />
          </div>
          <div>
          ...
          <button type="submit" disabled={loading} className="btn btn-primary" style={{ width: '100%', padding: '14px', marginTop: '10px' }}>
            {loading ? 'Entering...' : <><LogIn size={18} /> Sign In</>}
          </button>
        </form>

        <div style={{ marginTop: '30px', textAlign: 'center', fontSize: '14px', opacity: 0.5 }}>
          New student? <span onClick={onToggle} style={{ color: 'var(--accent-color)', cursor: 'pointer', fontWeight: 'bold' }}>Create Account</span>
        </div>
      </div>
    </div>
  );
