...
export const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ room, onDisconnect, lecture }) => {
  ...
  useEffect(() => {
    ...
    socket.on('sync_room_state', (state: any) => {
      console.log('[TEACHER-SYNC] Received Room State:', state);
      if (state.isRecordingAllowed !== undefined) {
        setIsRecordingAllowed(state.isRecordingAllowed);
      }
    });
  ...
  // MISSION 13: FAILSAFE - Close all screens if teacher tab is closed
  useEffect(() => {
  ...
  const handleToggleRecordingPermission = () => {
    ...
    socket?.emit('teacher:toggle_recording_permission', { 
      roomName: room.name, 
      allowed: nextAllowed 
    });
  };
